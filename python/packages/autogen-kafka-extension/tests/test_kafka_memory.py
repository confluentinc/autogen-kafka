...
@pytest.fixture
def kafka_memory_factory(kafka_connection: str):
    ...
    def create_kafka_memory(
        ...
        config = create_worker_config(kafka_connection, group_suffix, client_suffix)
...
async def setup_kafka_memory(kafka_memory: KafkaMemory) -> None:
    """Helper to start KafkaMemory and wait for initialization."""
    await kafka_memory.start()
    await asyncio.sleep(KAFKA_STARTUP_DELAY)

async def add_test_content(kafka_memory: KafkaMemory, content: str = TEST_CONTENT) -> None:
...
@pytest.mark.asyncio
async def test_distributed_add(kafka_memory_factory, memory: ListMemory):
    ...
    await setup_kafka_memory(kafka_memory_2)
    await setup_kafka_memory(kafka_memory_1)
    
    # Add content to first instance
    await add_test_content(kafka_memory_1)
...
@pytest.mark.asyncio
async def test_clear(kafka_memory_factory, memory: ListMemory):
...
@pytest.mark.asyncio
async def test_session_isolation(kafka_memory_factory):
    ...
    await add_test_content(kafka_memory_1, content_1)
    await add_test_content(kafka_memory_2, content_2)
...
@pytest.mark.asyncio
async def test_concurrent_operations(kafka_memory_factory, memory: ListMemory):
    ...
    await asyncio.gather(
        setup_kafka_memory(kafka_memory_1),
        setup_kafka_memory(kafka_memory_2),
        setup_kafka_memory(kafka_memory_3)
    )
    
    # Add content concurrently from different instances
    contents = ["concurrent_content_1", "concurrent_content_2", "concurrent_content_3"]
    await asyncio.gather(
        add_test_content(kafka_memory_1, contents[0]),
        add_test_content(kafka_memory_2, contents[1]),
        add_test_content(kafka_memory_3, contents[2])
    )
...
@pytest.mark.asyncio
async def test_metadata_handling(kafka_memory_factory, memory: ListMemory):
    ...
    await setup_kafka_memory(kafka_memory)
...
@pytest.mark.asyncio
async def test_query_edge_cases(kafka_memory_factory, memory: ListMemory):
...
@pytest.mark.asyncio
async def test_different_mime_types(kafka_memory_factory, memory: ListMemory):
...
@pytest.mark.asyncio
async def test_large_content(kafka_memory_factory, memory: ListMemory):
...
@pytest.mark.asyncio
async def test_multiple_add_same_session(kafka_memory_factory, memory: ListMemory):
...
@pytest.mark.asyncio
async def test_memory_persistence_across_restarts(kafka_memory_factory, memory: ListMemory):
