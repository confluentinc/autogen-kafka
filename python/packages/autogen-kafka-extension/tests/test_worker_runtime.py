...
@pytest.fixture
def worker_config_factory(kafka_connection):
    ...
    def _create_config(group_suffix: str = "test", client_suffix: str = "test"):
        return create_worker_config(kafka_connection, group_suffix, client_suffix)
    return _create_config


async def create_started_worker(kafka_connection: str, group_suffix: str = "test", client_suffix: str = "test"):
...
async def setup_multiple_workers(kafka_connection: str, worker_count: int = 2):
...
@pytest.mark.asyncio
async def test_agent_types_must_be_unique_single_worker(kafka_connection):
    """Test that agent types must be unique within a single worker."""
    worker = await create_started_worker(kafka_connection)
...
@pytest.mark.asyncio
async def test_agent_types_must_be_unique_multiple_workers(kafka_connection):
    """Test that agent types must be unique across multiple workers."""
    workers = await setup_multiple_workers(kafka_connection, 2)
...
@pytest.mark.asyncio
async def test_agent_type_register_instance(kafka_connection):
...
@pytest.mark.asyncio
async def test_agent_type_register_instance_different_types(kafka_connection):
...
@pytest.mark.asyncio
async def test_register_instance_factory(kafka_connection):
...
# Messaging Tests
@pytest.mark.asyncio
async def test_instance_factory_messaging(kafka_connection):
    ...
    config = create_worker_config(kafka_connection, "1", "1")
    worker = KafkaWorkerAgentRuntime(config=config)
...
@pytest.mark.asyncio
async def test_register_receives_publish(kafka_connection):
...
@pytest.mark.asyncio
async def test_disconnected_agent(kafka_connection):
